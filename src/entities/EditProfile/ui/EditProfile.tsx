...
export const EditProfile = ({ user }: EditProfileProps) => {
  const {
    ...
    { mode: 'onBlur', defaultValues: { email: user?.email ?? '', firstname: user?.profile?.firstname ?? '', lastname: user?.profile?.lastname ?? '', avatar: null } }
  );
  ...
  return (
    <form className={style.editProfile} onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className={style.editPhoto}>
        <Button className={style.userPhotoButton}>
