...
import { INotification, NotificationType, useNotification } from "@providers/NotificationsProvider";
import { CloseButton, CloseButtonSize } from "@shared/ui/CloseButton";
...
export const NotificationsItem = ({ id, type, message, timeout }: NotificationsItemProps) => {
  ...
  return (
    <div className={cn(style.notificationsItem, style[type])}>
      <CloseButton tabIndex={-1} onClick={() => removeNotification(id)} size={CloseButtonSize.S} className={style.close} />
